...
function Field({
  ...
  return (
    <div
      className={cn(
        'rounded-[var(--radius)] border border-border bg-card px-4 py-3.5 transition focus-within:border-[var(--brand-accent-strong)] focus-within:ring-2 focus-within:ring-ring/30 data-[invalid=true]:border-destructive data-[invalid=true]:ring-2 data-[invalid=true]:ring-destructive/20',
        className
      )}
      data-invalid={dataInvalid}
      {...rest}
...
function InputGroup({ children }: { children: React.ReactNode }) {
  return <div className="flex flex-1 overflow-hidden rounded-[var(--radius)] border border-input bg-background">{children}</div>
}
